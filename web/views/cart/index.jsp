...
<% String pageTitle = "Giỏ hàng | LinhNamStore"; %>
    ...
    <div style="text-align: right;">
        ...
        <div class="cart-actions-wrap">
            <form class="cart-qty-form" method="post" action="${pageContext.request.contextPath}/cart?action=update">
                ...
                <input type="number" min="1" max="<%= Math.max(item.getStock(), 1) %>" name="quantity" value="<%= item.getQuantity() %>" onchange="this.form.submit()">
...
(function () {
    ...
    function closeModal() { overlay.classList.add('is-hidden'); pendingForm = null; }
    function openModal(form) { pendingForm = form; titleEl.textContent = form.dataset.confirmTitle || 'Xác nhận'; msgEl.textContent = form.dataset.confirmMessage || 'Bạn có chắc muốn thực hiện thao tác này?'; overlay.classList.remove('is-hidden'); }

    forms.forEach(function (form) { form.addEventListener('submit', function (event) { if (form.dataset.confirmed === '1') { form.dataset.confirmed = '0'; return; } event.preventDefault(); openModal(form); }); });
    okBtn.addEventListener('click', function () { if (!pendingForm) return; pendingForm.dataset.confirmed = '1'; const form = pendingForm; closeModal(); form.requestSubmit(); });
    cancelBtn.addEventListener('click', closeModal);
    overlay.addEventListener('click', function (event) { if (event.target === overlay) closeModal(); });
    document.addEventListener('keydown', function (event) { if (event.key === 'Escape' && !overlay.classList.contains('is-hidden')) closeModal(); });
})();
</script>
