...
<c:set var="pageTitle" value="Lien he LinhNamStore" scope="request" />
<c:set var="activePage" value="contact" scope="request" />
<jsp:include page="../layouts/header.jsp" />
<h1 class="page-title">Lien he</h1>
<section class="two-column">
    <form class="panel form-grid" action="${pageContext.request.contextPath}/contact" method="post" data-validate>
        <input type="hidden" name="csrfToken" value="${sessionScope.csrfToken}">
        <div class="field"><label>Ho ten</label><input name="name" value="${submittedName}" required><span class="error"></span></div>
        <div class="field"><label>Email</label><input type="email" name="email" value="${submittedEmail}" required><span class="error"></span></div>
        <div class="field full"><label>Noi dung</label><textarea name="message" rows="6" required>${submittedMessage}</textarea><span class="error"></span></div>
        <button class="button" type="submit">Gui lien he</button>
